...
def pretrain_loop(model, dataloader, config, start_step=0):
    ...
    while global_step < config["num_train_steps"]:
        for batch in dataloader:
            ...
            if global_step % config.get("save_every", 2000) == 0:
                ckpt_path = f"new_checkpoints/bert_step_{global_step}.pt"
                save_checkpoint(model, scheduler, global_step, training_losses, ckpt_path)
    ...
    pbar.close()
    # final save
    save_checkpoint(model, scheduler, global_step, training_losses, "new_checkpoints/bert_final.pt")
    print("Training finished. Model saved to new_checkpoints/bert_final.pt")
